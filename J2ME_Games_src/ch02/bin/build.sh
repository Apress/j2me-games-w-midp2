...
$JAVA4_HOME/jar cmf MANIFEST.MF maze.jar -C ../classes .

echo "Updating JAR size info in JAD file..."

NB=`wc -l maze.jad | awk '{print $1}'`
head --lines=$(($NB-1)) maze.jad > maze.jad1
echo "MIDlet-Jar-Size:" `stat -c '%s' maze.jar`>> maze.jad1
cp maze.jad1 maze.jad
